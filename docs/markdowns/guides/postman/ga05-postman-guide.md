...
- **Method**: `POST`
- **URL**: `{{base_url}}/emails/sync?accountId={{account_id}}`
- **Auth**: Inherit (Bearer Token)
- **Kỳ vọng**: 
  - Status `200 OK`.
  - Body: `{"success": true, "message": "Sync completed"}`.
- **Tác dụng**: Hệ thống sẽ tải 50 email mới nhất từ INBOX về và lưu vào bảng `emails` với status mặc định `INBOX`.
...
## 4. AI Summarization (Tóm tắt nội dung)

Hệ thống hỗ trợ 3 mức độ ưu tiên:
1. **Cloud API** (OpenAI/Gemini): Cần API Key. Chất lượng tốt nhất.
2. **Local AI** (Ollama): Cần chạy Ollama. Riêng tư, không tốn phí.
3. **Extractive (Fallback)**: Thuật toán tách câu quan trọng (Java port từ Go). Luôn hoạt động.

- **Method**: `POST`
- **URL**: `{{base_url}}/emails/{{email_id}}/summarize`
- **Auth**: Inherit
- **Kỳ vọng**: 
  - Status `200 OK`.
  - Trả về `summary`: "Tóm tắt ngắn gọn của email...".
  - Field `summary` được lưu vào DB.

---

## 5. Test Kịch bản (Scenario)
...
4. **AI**: Gọi API Summarize (4) cho email đó. Kiểm tra field `summary` được trả về.
5. **Snooze**: Gọi API Snooze (3) với thời gian tương lai gần (VD: 1 phút nữa).
...
- **Lỗi AI không chạy?**
  ...
  - Nếu muốn test Fallback, hãy xóa/đổi tên key này và gọi lại API.
