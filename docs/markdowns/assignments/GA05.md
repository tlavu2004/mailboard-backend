...
<table>
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>The interface renders the board with distinct columns (flexible configuration allowed, e.g., Inbox, To Do, Done).</li>
        <li>Cards display <b>real</b> email data fetched from the backend (must include Sender, Subject, and Content snippet).</li>
        <li>The layout is organized and visually readable (Kanban style).</li>
      </ul>
    </td>
    <td><b>25</b></td>
  </tr>
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>Users can successfully drag a Card from one column to another.</li>
        <li>Dropping a Card triggers a Backend update to change the email's status.</li>
        <li>The UI updates the Card's position immediately without requiring a full page refresh.</li>
      </ul>
    </td>
    <td><b>25</b></td>
  </tr>
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>Selecting "Snooze" correctly removes or hides the Card from the active column (e.g., Inbox).</li>
        <li>The Card is successfully moved to a "Snoozed" state or column.</li>
        <li>Logic is implemented to "wake up" (restore) the email to the active view after the simulation or time passes.</li>
      </ul>
    </td>
    <td><b>25</b></td>
  </tr>
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>The backend successfully sends real email text to a processing API (LLM or library).</li>
        <li>The system returns a dynamically generated summary (no hardcoded or mock text allowed).</li>
        <li>The summary is clearly displayed on the Card or in a detailed view.</li>
      </ul>
    </td>
    <td><b>25</b></td>
  </tr>
